...
- Use `@openagent/test-utils` for common mocks (`createMockProvider`, `createMockTool`, etc.).
...
Build order is determined by inter-package dependencies. For example, `@openagent/core` depends on `@openagent/llm` and `@openagent/tools`, so those are built first.
